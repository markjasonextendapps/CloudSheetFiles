...
// New License
GC.Spread.Sheets.LicenseKey = "EXTENDAPPS,E773374745574389##";
...
function paintSheetData(data, newSheet){
    ...
    for(var i = 1; i <= columnCount; i++){
        try {
            ...
            sheet.setValue(4, i, data.columns[i - 1].label);
            for(var k = 0; k< data.result.length; k++) {
                sheet.setValue(5+k, i, data.result[k].values[data.columns[i - 1].name]);
            }
    ...
    //GcSpread.Sheets.LineBorder("#7FFFD4",GcSpread.Sheets.LineStyle.medium), {all:true},3);
    //sheet.setBorder(sheet.getRange(1, 1, 5+data.result.length, Math.max(columnCount,2)), border, {outline:true}, 3);
    // .borderBottom(border).borderTop(border).borderLeft(border).locked(true);
...
function prepareSavedSearchBuilder($container) {
    ...
    function changeSavedSearchDescription(name) {
        ...
        if (name && name != "") {
            html = "<div class='function-description-container'><strong>"+name+"</strong><br>" + ["[", scriptid, "]"].join("") + "</div> " ;
        }else{
