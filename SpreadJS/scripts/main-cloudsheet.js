let accountId, scriptId, scriptDeployId, workbookId;
let appVersionNumber = "0.0.1";
let savedWorkbooktData = null;
let workbookInputName = null;
let selectedSearchId = null;
const savedSearchDs =[];
let isFullscreen;
let isUserWorkbookOwner = false;
let userHaveWorkbookWriteAccess = false;
let spreadsheet = null;
let savedSearchesList = null;
let savedSearchesListMap = {};
let employeeList = null;
let shareDialog = null;
let readonlyMultiSelect = [];
let readwriteMultiSelect = [];
let readCombinedMultiSelect = {};
let worksheetEdited = false;
...
scriptId = jQuery.urlParam('script');
scriptDeployId = jQuery.urlParam('deploy');
workbookId = jQuery.urlParam('workbookId');
accountId = jQuery.urlParam('accountId');
...
if(isFullscreen){
    ...
    jQuery('.content-container').css('max-height','100%');

    var iframe = jQuery('#custpage_iframe_val',window.parent.document).children();
    iframe.css( "height", "90%" );
    iframe.css( "max-height", "90%" );

}


const mainSuiteletUrl = "https://"+accountId+".app.netsuite.com/app/site/hosting/scriptlet.nl?script=customscript_cs_spreadsheet_su&deploy=customdeploy_cs_spreadsheet_su";
const listSuiteletUrl = "https://"+accountId+".app.netsuite.com/app/site/hosting/scriptlet.nl?script=customscript_cs_list_workbooks_su&deploy=customdeploy_cs_list_workbooks_su";

const arrAtoZZ = [];
const arrAtoZ = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
arrAtoZ.forEach(v1=>{
    arrAtoZZ.push(v1);
});
arrAtoZ.forEach(v1=>{
...
const objAtoZZ = arrAtoZZ.reduce((acc, val, idx) => {
    acc[idx] = val;
    return acc;
}, {});
...
window.addEventListener("beforeunload", function(e){
    ...
    var confirmationMessage = "Leave without saving changes?";
    (e || window.event).returnValue = confirmationMessage;
    return confirmationMessage;
});
jQuery("#readonlyselect").on('click', function (e) {
    userHaveWorkbookWriteAccess = false;
    console.log('readonlyselect');
});
jQuery("#readwriteselect").on('click', function (e) {
    userHaveWorkbookWriteAccess = true;
    console.log('readwriteselect');
});
...
function toggleState() {
    var $element = $(this),
        ...
        $content = $parent.siblings(".pane-group-content"),
...
function showColorPicker() {
    ...
    if ($target) {
        ...
        if (top < MIN_TOP) {
            ...
            var $inspContainer = $("#setting-container"),
...
function fetchWorkbook(){
    return fetch(mainSuiteletUrl, {
        method: 'POST',
        body: JSON.stringify({id: workbookId, action: 'loadWorkbook'})
    });
}

function fetchWorkbookList(){
    return fetch(listSuiteletUrl, {
        method: 'POST',
        body: JSON.stringify({action: 'loadWorkbook'})
    });
}
...
function fetchSavedSearches(){
    return fetch(mainSuiteletUrl, {
        method: 'POST',
        body: JSON.stringify({action: 'getSavedSearchList'})
    });
}
...
function fetchEmployeeList(){
    return fetch(mainSuiteletUrl, {
        method: 'POST',
        body: JSON.stringify({action: 'searchEmployees'})
    });
}

function getSavedSearchResult(scriptid){
    return fetch(mainSuiteletUrl,{
        method: 'POST',
        body : JSON.stringify({
...
function deleteWorkbook(){
    return fetch(mainSuiteletUrl,{
        method: 'POST',
        body : JSON.stringify({id: savedWorkbooktData.id, action: 'deleteWorkbook'})
    });
}

function initNewSpread() {
    console.log('--- initNewSpread ---')
    var sheet = spread.getActiveSheet();
    sheet.suspendPaint();
        ...
        try {
            ...
            fetchWorkbook()
                ...
                .then(responseJson => {
                    ...
                    isUserWorkbookOwner = responseJson.access === 'owner';
                    userHaveWorkbookWriteAccess = isUserWorkbookOwner || responseJson.access === 'readwrite';

                    spread.fromJSON( responseJson.data );
                    savedWorkbooktData = {
                        name: responseJson.name,
                        id: responseJson.id
                    };
                    if(responseJson.name) {
                        jQuery("#saveAsFileName2").val(responseJson.name);
                        jQuery("#cloudSheetTitle").text(responseJson.name + ' -  CloudSheet');
                    }

                    addSaveBtn();


                    if(responseJson.share){
                        (responseJson.share).forEach( result => {

                            var employeeid = result.id;
                            var type = result.type;
                            readCombinedMultiSelect[employeeid] = result;
                            console.log(employeeid);
                            if(type == 'readonly'){
                                jQuery('.share-input[data-employee="'+employeeid+'"]').prop( "checked", true );

                            }
                            if(type == 'readwrite'){
                                jQuery('.share-input[data-employee="'+employeeid+'"]').prop( "checked", true );
                                jQuery('.share-input-write[data-employee="'+employeeid+'"]').prop( "checked", true );
                            }
                        });
                    ...
                    if (userHaveWorkbookWriteAccess) {
                        /*
                        const sheets = spreadsheet.sheets();
                        responseJson.savedsearch.forEach(lssd => {
                            savedSearchDs.push({
                                id: lssd.searchId,
                                sheet: sheets[lssd.sheetIndex],
                                columncount: lssd.columncount,
                                rowcount: lssd.rowcount
                            });
                        });
                    ...
                    if (isUserWorkbookOwner) {
                        ...
                        const readwriteValues = [];
                        for(let idx=0; idx<responseJson.share.length; idx++){
                            if(responseJson.share[idx]['type'] === 'readonly'){
                                readonlyValues.push({
                                    name : responseJson.share[idx]['name'],
                                    id   : responseJson.share[idx]['id']
                                });
                            }
                            if(responseJson.share[idx]['type'] === 'readwrite'){
                                readwriteValues.push({
                                    name : responseJson.share[idx]['name'],
                                    id   : responseJson.share[idx]['id']
                                });
                            }
                        ...
                        readonlyValues.forEach( value => {
                            readonlyMultiSelectDs.add(value);
                        });
                        readonlyMultiSelect.value(readonlyValues.map(v=>`${v.id}`));
                        readonlyMultiSelect.trigger('change');
                        readonlyMultiSelect.refresh();
                        readwriteValues.forEach( value => {
                            readwriteMultiSelectDs.add(value);
                        });
                        readwriteMultiSelect.value(readwriteValues.map(v=>`${v.id}`));
                        readwriteMultiSelect.trigger('change');
                        readwriteMultiSelect.refresh();
                    */
                    }
                })
                .catch(error => {
...
function refreshSavedSearch(){
    var sheet = spread.getActiveSheet();
    var urlString = sheet.getCell(2,1,GC.Spread.Sheets.SheetArea.colHeader).value();
    var scriptid = getSearchId(urlString);
...
function triggerRefreshAllSearch(){
    showLoading();
    refreshAllSavedSearch().then(response => {
        return response;
    }).finally(() => {
        recalculateAll();
        hideLoading();
...
function refreshAllSavedSearch(){
            ...
            if (+scriptid > 0) {
                getSavedSearchResult(scriptid).then(response => {
                    ...
                    if (data.error) {
                        ...
                        paintSheetData(data, false, sheet);
    ...
    Promise.all(promises)
        ...
        .finally(() => {
            console.log('time x:'+new Date().getTime());
            recalculateAll();
            hideLoading();
            console.log('time z:'+new Date().getTime());
        });


}

function getSearchId(urlString){
    var searchId = null;
    console.log(urlString);
    try{
        if (urlString != null) {
            var url = urlString.split('?id=');
            url.length > 1? searchId = url[1] : searchId = null;
        }
    }catch(e){
...
function refreshSpecificSheet(index){
    var success = false;
    try {
...
function recalculateAll(){
    ...
    var currentIndexId = spread.getActiveSheetIndex();
    for(var i = 0; i < spread.getSheetCount(); i++) {
        try {
            spread.setActiveSheetIndex(i);
            var sheet = spread.getActiveSheet();
    ...
    spread.setActiveSheetIndex(currentIndexId);

}

function setName(sheet, sheetTitle){

    var modifiedTitle = sheetTitle.replace('/^[\\w\\-\\s]+$/', "").substring(0, 30);
    ...
    sheet.name(modifiedTitle);

}


function paintSheetData(data, newSheet, specificSheet) {
    if (newSheet == true) {
        spread.addSheet();
    ...
    for(var i = 0; i < (data.columns.length || 0); i++){
        try {
    ...
    sheet.setDataSource(data.result, true);
    sheet.setRowCount(4, GC.Spread.Sheets.SheetArea.colHeader);

    sheet.addSpan(0, 0, 1, Math.max(columnCount,2), GC.Spread.Sheets.SheetArea.colHeader);
    sheet.setRowHeight(0, 45, GC.Spread.Sheets.SheetArea.colHeader);
    sheet.getCell(0, 0, GC.Spread.Sheets.SheetArea.colHeader).value( data.title).foreColor("Background 1");
    sheet.getRange(0, 0, 1, Math.max(columnCount,2), GC.Spread.Sheets.SheetArea.colHeader).backColor("Accent 6 -50").foreColor("Background 1").hAlign(0).font("29.3333px Arial");
    sheet.getCell(1, 0, GC.Spread.Sheets.SheetArea.colHeader).value( "Date: ");

    sheet.addSpan(1, 1, 1, Math.max(columnCount-1,1), GC.Spread.Sheets.SheetArea.colHeader);
    sheet.getRange(1, 0, 1, Math.max(columnCount,2), GC.Spread.Sheets.SheetArea.colHeader).backColor("Accent 6 -50").foreColor("Background 1").hAlign(0).font("11px Arial");
    sheet.getCell(1, 1, GC.Spread.Sheets.SheetArea.colHeader).value( data.date);

    sheet.getCell(2, 0, GC.Spread.Sheets.SheetArea.colHeader).value( "Search URL:"); //json2
    sheet.addSpan(2, 1, 1, Math.max(columnCount-1,1), GC.Spread.Sheets.SheetArea.colHeader);
    sheet.getRange(2, 0, 2, Math.max(columnCount,2), GC.Spread.Sheets.SheetArea.colHeader).backColor("Accent 6 -50").foreColor("Background 1").hAlign(0).font("11px Arial");
    sheet.getCell(2, 1, GC.Spread.Sheets.SheetArea.colHeader).value(data.url); //json2

    for(var i = 0; i < columnCount; i++){
        try {
            sheet.getCell(3, i, GC.Spread.Sheets.SheetArea.colHeader).value( data.columns[i].label); //json2
            // sheet.setValue(4, i,,GC.Spread.Sheets.SheetArea.viewport,true);
    ...
    var columns = sheet.getColumnCount();
    sheet.addRows(rows, 1000);
    sheet.addColumns(columns, 200);
    ...
    /*
      ...
      sheet.getRange(4, 1, 1, Math.max(columnCount,2)).backColor("Accent 6 -50").foreColor("Background 1").hAlign(0).font("11px Arial");
      sheet.setValue(2, 1, "Date: ");
      sheet.setValue(2, 2, data.date);
      sheet.setValue(3, 1, "Search URL:"); //json2
      var cellLink = new GC.Spread.Sheets.CellTypes.HyperLink();
      cellLink.linkColor("rgb(255, 255, 255)");
      cellLink.visitedLinkColor("rgb(255, 255, 255)");
      cellLink.text(data.url);
      ...
      for(var i = 1; i <= columnCount; i++){
          try {
              ...
              sheet.setValue(4, i, data.columns[i - 1].label,GC.Spread.Sheets.SheetArea.viewport,true);
              for(var k = 0; k< data.result.length; k++) {
                  sheet.setValue(5+k, i, data.result[k].values[data.columns[i - 1].name],GC.Spread.Sheets.SheetArea.viewport,true);
              }
              sheet.autoFitColumn(i);
      ...
      sheet.getRange(1, 1, 4+data.result.length, Math.max(columnCount,2))
          .setBorder(
...
function saveWorkbook( workbookName ){
    ...
    return new Promise(function(resolve,reject){
        ...
        .then(rjd=> {
            if (rjd.error) {
                hideLoading();
                console.log('rjd.error', rjd.error);
                jQuery('#popupModalTitle').html(rjd.error);
                jQuery('#popupModal').modal('show');
                return Promise.reject(rjd.error);
            } else {
                if(savedWorkbooktData) {
                    if(workbookName){
                        console.log('Save As New Workbook');
                        let currentUrl = window.parent.location.href;
                        let newUrl = currentUrl.replace('workbookId='+savedWorkbooktData.id,'workbookId='+rjd.id);
                        window.parent.location = newUrl;
                        savedWorkbooktData.id = rjd.id;
                        savedWorkbooktData.name = workbookName;
                    } else {
                        // console.log('Save existing Workbook');
                        jQuery('#popupModalTitle').html('Save existing Workbook');
                        jQuery('#popupModal').modal('show');
                    }
                } else {
                ...
                addSaveBtn();
                if(workbookName) {
                    jQuery("#cloudSheetTitle").text(workbookName + ' -  CloudSheet');
                }


                $('#file-list').html('<table id="file-list" class="table table-condensed table-hover mb-0"></table>');
                ...
                fetchWorkbookList()
                    ...
                    .then(responseJson => {
                        ...
                        for(var i = 0; i < responseJson.length; i++){
                            var woorkBookObject = responseJson[i];
                            $('#file-list').append('<tr><td><a href="#" onClick="window.parent.location=\''+woorkBookObject.view+'\'">'+woorkBookObject.name+'</a></td></tr>');
                ...
                jQuery('#popupModalTitle').html('CloudSheet Saved.');
                jQuery('#popupModal').modal('show');
            }
        });
...
function addSaveBtn(){
    // Check if save button is already added
    if(!jQuery('#filemenu-save').length){
        jQuery('#filemenu-open').closest('li').after('<li><a id="filemenu-save" href="#" class="localize">Save</a></li>');
        jQuery("#filemenu-save").on('click', function (e) {
            var filename = $("#saveAsFileName2").val() || "export";
            saveWorkbook();
...
function deleteWorkbookBtnClicked(){
    if (!isUserWorkbookOwner) {
        alert(`You don't have access to delete this Workbook.`);
        return;
    }
    var confirmDelete = confirm("Are you sure that you want to delete this cloudsheet?");
        ...
        if(savedWorkbooktData){
            deleteWorkbook()
                ...
                .then(responseJson=>{
                    ...
                    window.parent.location = responseJson.redirecturl;
                })
                .catch(error=>{
                    console.log('load workbook error', error);
                    alert(error);
                });
...
function processFileSelected() {
    ...
    console.log('processFileSelected --------');
...
function attachSpreadEvents(rebind) {
    ...
    spread.bind(spreadNS.Events.ActiveSheetChanged, function () {
        ...
        console.log('---- active sheet change ----' + rowCount);
        if(rowCount < 999){
...
function setDataValidator() {
    ...
    if (currentDataValidator) {
        ...
        console.log(' errorStyle '+getDropDownValue("errorAlert"));
...
$(document).ready(function () {
    ...
    fetchSavedSearches()
        ...
        .then(responseJson => {
            savedSearchesList = responseJson;
            prepareSavedSearchBuilder($("#addSavedSearchBuilder"));
        });
    ...
    fetchEmployeeList()
        ...
        .then(responseJson => {
            ...
            prepareShareBuilder($("#addShareBuilder"));
        });
    ...
    fetchWorkbookList()
        ...
        .then(responseJson => {
            ...
            for(var i = 0; i < responseJson.length; i++){
                var woorkBookObject = responseJson[i];
                jQuery('#file-list').append('<tr><td><a href="#" onClick="window.parent.location=\''+woorkBookObject.view+'\'">'+woorkBookObject.name+'</a></td></tr>');
    ...
    spread.bind(GC.Spread.Sheets.Events.SheetTabClick, function (e, info) {
        ...
        if (info.sheet === null && info.sheetName === null) {
            ...
            initNewSpread();
            console.log("New button Clicked!! " + JSON.stringify(info));
        }
        else {
    ...
    spread.bind(GC.Spread.Sheets.Events.SheetNameChanged, function (sender, args) {
        var name = args.newValue;
        if(name.length > 30){
            var sheet = spread.getActiveSheet();
            sheet.name(name.substring(0, 30));
            alert('Maximum length for sheet name is 30. Name is truncated. ');
        }
    });
...
function processRibbonClick(e, data) {
    ...
    switch (name) {
        ...
        case "cellsgroup":
            ...
            break;
        case "textformat":
            processTextFormat(value, toggle);
            break;
        ...
        case "conditionalformat":
            displaySettingPane(title, $('#conditionFormattingPane'));
        ...
        case "insertSavedSearch":
            displaySettingPane(title, $('#addSavedSearchBuilder'));
            break;
        case "undoCloudSheet":
        ...
        case "saveCloudSheet":
            var filename = $("#saveAsFileName2").val();
            console.log(filename);
            if(!filename || filename.trim() == '' || filename.trim() == "export") {
                app.fileMenu.showFileScreen();
            }else{
                saveWorkbook();
            }
            break;
        case "deleteCloudSheet":
            deleteWorkbookBtnClicked();
            break;


        case "shareCloudSheet":
            displaySettingPane(title, $('#addShareBuilder'));
        ...
        case "fullScreen":
            ...
            let newUrl = '';
            if(isFullscreen){
                newUrl = currentUrl.replace('&fullscreen=T','');
            } else {
                let match = currentUrl.match('[?&]' + 'fullscreen' + '=([^&]+)');
                newUrl = match ? currentUrl.replace(match[0], '') : currentUrl;
                newUrl += '&fullscreen=T';
            }
            var confirmFullScreen = confirm("Toggle Screen Mode: Unsaved changes will be lost when toggling between fullscreen and standard screen modes.");
            if (confirmFullScreen == true) {
...
function processTextFormat(name, value) {
    console.log('processTextFormat--'+name);
...
function CopyAndinsert() {
    ...
    let name = "Sheet" + +(sheetCount + 1);
    newSheet.suspendPaint();
    newSheet.options.isProtected = false;
    newSheet.resumePaint();
    newSheet.name(name);
    spread.addSheet(sheetCount, newSheet);
    spread.setActiveSheet(name);
}

function prepareSavedSearchBuilder($container) {

    //json
    function filterSearches($ul, s) {
        // restore all
        $ul.children().removeClass("hidden");
        // do filter when filter string provided
        if (s) {
            ...
            s = s.toUpperCase();
            // filter functions by name, hide not matched items
            $("li.savedsearch-item", $ul).filter(function () {
                var funcName = $(this).text().toUpperCase();
                return funcName.indexOf(s) == -1; // or don't use startsWith:  funcName.indexOf(s) != 0;
            }).addClass("hidden");

            // hide category without visible function items
            $("li.savedsearch-category", $ul).filter(function () {
                var name = $(this).data("name");
                return $("li.savedsearch-item[data-category='" + name + "']:visible", $ul).length === 0;
            }).addClass("hidden");
        }
    }
    ...
    for (var name in savedSearchesList) {
    ...
    $("li.savedsearch-item", $ul).click(function () {
        var name = $(this).text();
        $('li.selected', $ul).removeClass("selected");
        $(this).addClass("selected");
        //addSavedSearchToSheet(name);
        changeSavedSearchDescription(name);
    });

    $("li.savedsearch-item", $ul).dblclick(function () {
        var name = $(this).text();
    ...
    $("input", $container)
        ...
        .keyup(function () {
            filterSearches($ul, $(this).val());
        });

    function changeSavedSearchDescription(name) {
        ...
        $("#addSavedSearchBuilder .savedsearch-description").html(html);
    }

    jQuery("#addSavedSearchButton").on("click", function () {
...
function prepareShareBuilder($container) {

    //json
    function filterList(s) {
        // restore all
        $(".share-row").removeClass("hidden");
        // do filter when filter string provided
        if (s) {
            // to upper for match with function name
            s = s.toUpperCase();
            // hide not matched items
            $(".share-row").filter(function () {
                var employeeName = $(this).text().toUpperCase();
                return employeeName.indexOf(s) == -1;
            }).addClass("hidden");

        }
    }

    jQuery('#employee-list').append("<tr><b><td style='word-break:break-word;'><label class='pane-label localize'></label></td><td style='word-break:break-word;margin-left: -12px'><label class='localize'>Read</label></td><td style='word-break:break-word;'><label class='localize'>Write</label></td></b></tr>");

    for (var name in employeeList) {
        var cat = employeeList[name], text = cat.name, id = cat.id;

        jQuery('#employee-list').append("<tr class='share-row' id='"+text+"'><td style='word-break:break-word;'><label class='pane-label localize'>"+text+"</label></td><td style='word-break:break-word;'><input type='checkbox' class='share-input localize-value' data-employee="+id+"></td><td style='word-break:break-word;'><input type='checkbox' class='share-input-write localize-value'  data-employee="+id+"></td></tr>");
    ...
    console.log('readCombinedMultiSelect Init', JSON.stringify(readCombinedMultiSelect));

    jQuery("#addShareButton").on("click", function () {
        readCombinedMultiSelect = {};
        var selected = jQuery(".share-input:checkbox:checked");
        if(selected && selected.length > 0) {

            for(var i = 0; i < selected.length; i++){
                var employeeid = jQuery(selected[i]).data('employee');
                readonlyMultiSelect.push({type: 'readonly', id: employeeid});
                readCombinedMultiSelect[employeeid] ={type: 'readonly', id: employeeid};
        ...
        var selectedWrite = jQuery(".share-input-write:checkbox:checked");
        if(selectedWrite && selectedWrite.length > 0) {

            for(var i = 0; i < selectedWrite.length; i++){
                var employeeid = jQuery(selectedWrite[i]).data('employee');
                readwriteMultiSelect.push({type: 'readwrite', id: employeeid});
                readCombinedMultiSelect[employeeid] ={type: 'readwrite', id: employeeid};
        ...
        console.log('readCombinedMultiSelect', JSON.stringify(readCombinedMultiSelect));
        jQuery('#popupModalTitle').html('Permission added to sheet');
        jQuery('#popupModal').modal('show');
        app.fileMenu.closeFileScreen();
    });
}
